...
| Первый экран | Последние комнаты |
|---|---|
| ![Первый экран: штрихи рисуются сами](docs/shots/hero.png) | ![Комнаты, где вы уже были](docs/shots/rooms.png) |

| Дневной режим | С телефона |
|---|---|
| ![Светлая тема без свечения](docs/shots/day.png) | ![Панель переносится по строкам](docs/shots/mobile.png) |
...
| | |
|---|---|
| **Блокнот и холст вместе** | По умолчанию это блокнот: пишешь текст. Перо и ластик включаются кнопкой — холст лежит поверх и не мешает набору |
...
| **Горячие клавиши** | `T` — текст, `B` — перо, `E` — ластик (в поле ввода не срабатывают) |
| **Последние комнаты** | Комнаты, где вы уже были, запоминаются в браузере и ждут на главной |
